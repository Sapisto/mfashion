  ...
  try {
    ...
    await Promise.all([
      ...
      resend.emails.send({
        from: FROM,
        to: ADMIN_EMAIL,
        subject: `🛍️ New Order #${shortId} — ${new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN", minimumFractionDigits: 0 }).format(data.total)}`,
        html: await render(
          NewOrderAlertEmail({
            customerName: data.customerName,
    ...
    // Email errors should never break the order flow
